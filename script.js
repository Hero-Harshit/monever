...
function renderPeople() {
  ...
  if (netBalance > 0) {
    netItem.innerHTML = `<span>You are owed ${getCurrency()}${formatMoney(netBalance)} net</span>`;
  } else if (netBalance < 0) {
    netItem.innerHTML = `<span>You owe ${getCurrency()}${formatMoney(Math.abs(netBalance))} net</span>`;
  } else {
  ...
  chartInstance = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: [
          '#404040', '#606060', '#808080', '#A0A0A0', '#C0C0C0', '#D0D0D0', '#707070'
        ],
        borderWidth: 1,
        borderColor: '#ffffff',
        hoverOffset: 0
